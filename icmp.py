...
MAX_SLEEP = 1000
file1 = "/myfiles/request.txt"
file2 = "/myfiles/reply.txt"
f1 = open(file1, "a+")
f2 = open(file2, "a+")
...
def do_one(myStats, destIP, hostname, timeout, mySeqNumber, packet_size, quiet=False):
    ...
    try:  # One could use UDP here, but it's obscure
        ...
        f2.write("failed. (socket error: '%s')" % e.args[1])
    ...
    if recvTime:
        ...
        if not quiet:
            ...
            f2.write("%d bytes from %s: icmp_seq=%d ttl=%d time=%d ms\n" % (
                dataSize, socket.inet_ntoa(struct.pack("!I", iphSrcIP)), icmpSeqNumber, iphTTL, delay)
            )
        myStats.pktsRcvd += 1
...
def send_one_ping(mySocket, destIP, myID, mySeqNumber, packet_size):
    ...
    try:
        ...
        f1.write("General failure (%s)" % (e.args[1]))
        return
...
if __name__ == '__main__':
    ...
    with open("/myfiles/request.txt", "r") as req:
        for r in req:
            print(r)
    print("\nPrinting reply from the file")
    with open("/myfiles/reply.txt", "r") as rep:
        for r in rep:
            print(r)